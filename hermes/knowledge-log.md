...
<!-- KNOWLEDGE id=2026-05-31T05:11:12Z -->
### 2026-05-31T05:11:12Z — Vercel deploy failure: sub-daily cron on Hobby
GOTCHA: Vercel Hobby plan rejects any cron schedule more frequent than once/day and FAILS the whole deployment. Phase 4 added crons '*/5 * * * *' to vercel.json, which made every Phase 4 deploy fail — production silently kept serving the pre-Phase-4 build (PR #5 / 39982c1), so /api/budget 404'd in prod even though the code was merged to master and builds green. Fix (PR #7): change schedule to '0 9 * * *' (once daily, 09:00 UTC) — deploys on any plan. Trade-off: staleness/budget/failed-run Telegram alerts fire once a day instead of every 5 min; budget gauge + kill switch + SSE monitor are unaffected (cron-independent). If on Pro, can tighten again; or drive the 5-min staleness check from the PC scheduler. Diagnosis pattern: green 'next build' + 'Deployment failed' on Vercel = deploy-config issue (cron/functions/regions), not code.
